...
exports.default = (req, res, next) => {
    res.reply = (responseJson) => {
        let data = typeof responseJson.data === "undefined" ? {} : responseJson.data;
        let statusCode = typeof responseJson.statusCode === "undefined"
            ? 200
            : responseJson.statusCode;
        let message = typeof responseJson.message === "undefined"
            ? generateMessage(statusCode)
            : responseJson.message;
        let statusCodeText = http_status_codes_1.default.getStatusText(statusCode);
        ...
        if (statusCode >= 300) {
            res.status(statusCode).send(Object.assign({}, result, { status: false, error: data }));
            return;
        }
        res.status(statusCode).send(Object.assign({}, result, { status: true, data: data }));
        return;
    };
    next();
};
